...
def matakuliah_add(request):
    ...
    try:
        ...
        try:
            ...
            if sks <= 0:
                return HTTPBadRequest(json_body={'error': 'SKS harus bernilai positif'})
            if semester <= 0:
                return HTTPBadRequest(json_body={'error': 'Semester harus bernilai positif'})
        except ValueError:
...
def matakuliah_update(request):
    ...
    try:
        ...
        # Validasi dan update data numerik
        if 'sks' in json_data:
            try:
                sks = int(json_data['sks'])
                if sks <= 0:
